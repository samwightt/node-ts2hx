...
class HXDumper {
  ...
  addContextType(name, type) {
    this.context["type:" + name] = type;
  }

  addRootContextType(name, type) {
  ...
  dumpImport(element) {
    ...
    if (
      ...
      element.moduleReference.moduleName != null
    ) {
      ...
      if (moduleReferenceLastPart.indexOf(".") != -1) {
        ...
        moduleReferenceLastPart =
          moduleReferenceLastPart[moduleReferenceLastPart.length - 1];
      }
    }
    ...
    if (element.identifier != null && moduleReferenceLastPart != null) {
      ...
      if (identifier != moduleReferenceLastPart) {
        this.writeIndentedLine(
          "typedef " + identifier + " = " + moduleReferenceLastPart + ";"
        );
      }
    }
  }
  ...
  dumpHeritageClauses(heritageClauses) {
  ...
  dumpClass(element) {
    if (
      ...
      !this.classHasStaticMethodsWithTypeParameterList(element)
    ) {
      // Generic?
    ...
    if (element.typeParameterList) {
      this.classGenericTypes = this.dumpTypeParameterList(
        element.typeParameterList
      );
    } else {
  ...
  classHasStaticMethodsWithTypeParameterList(classInfo) {
  ...
  dumpClassElements(elements) {
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      if (
        ...
        element.variableDeclarator.propertyName
      ) {
        // Class property
  ...
  dumpClassProperty(element) {
    ...
    if (
      element.variableDeclarator.typeAnnotation &&
      element.variableDeclarator.typeAnnotation.type
    ) {
      var type = this.type(element.variableDeclarator.typeAnnotation.type);
      if (type) {
        this.addRootContextType(
    ...
    // Default value
    if (
      element.variableDeclarator.equalsValueClause &&
      element.variableDeclarator.equalsValueClause.value
    ) {
      // Compute type from value
      if (!didComputeType) {
        // Try to infer type from assignment if possible
        if (!didComputeType) {
          var type = this.typeFromValue(
            element.variableDeclarator.equalsValueClause.value
          );
          if (type != null) {
            this.addRootContextType(
  ...
  dumpClassPropertyFromAccessor(element) {
  ...
  dumpClassPropertiesFromConstructor(element) {
    if (
      element.callSignature &&
      element.callSignature.parameterList &&
      element.callSignature.parameterList.parameters
    ) {
  ...
  dumpClassPropertiesAssignmentsFromConstructor(element) {
    if (
      element.callSignature &&
      element.callSignature.parameterList &&
      element.callSignature.parameterList.parameters
    ...
    if (element.block) {
      ...
      if (!this.inClosure) {
        ...
        if (
          ...
          this.hasClosuresInStatements(element.block.statements)
        ) {
          hasClosures = true;
  ...
  dumpClassMethod(element) {
    ...
    if (modifiers["override"]) {
      ...
      if (this.className && this.info.classes[this.className]) {
        for (var parentClass in this.info.classes[this.className]
          ...
          if (
            (classInfo != null && classInfo.methods[methodName]) ||
    ...
    // Type
    if (
      element.callSignature.typeAnnotation &&
      element.callSignature.typeAnnotation.type
    ) {
    ...
    if (element.block) {
      ...
      if (element.setKeyword) {
        if (element.callSignature.parameterList.parameters.length > 0) {
          var name =
            element.callSignature.parameterList.parameters[0].identifier;
          if (name) {
  ...
  parentHasMethod(className, methodName) {
  ...
  dumpStatements(statements, dontFinishWithLineBreak) {
  ...
  hasClosuresInStatements(statements) {
  ...
  dumpPropertyAssignments(assignments) {
  ...
  updateLineMappingWithInput(input, customFullStart) {
    if (input == null) return;

    if (!this.isExtractingValue) {
      var fullStart = input._fullStart;
      if (fullStart == null) {
      ...
      if (this.lineMapping[String(dstCurrentLine)] == null) {
        var srcCurrentLine = this.ast.text.value
          .substring(0, fullStart + 1)
          .split("\n").length;
  ...
  dumpValue(input, options) {
    ...
    if (input.cachedText) {
      ...
      if (this.isReferencingStructureKeyWithString(input)) {
        ...
        if (
          !this.strict &&
          !this.isPerformingArrayAccessWithIntegerIdentifier(input)
        ) {
      ...
      if (input.variableDeclaration) {
        if (input.forKeyword && input.inKeyword) {
          var variableDeclarators =
            input.variableDeclaration.variableDeclarators;
          if (
            !variableDeclarators &&
            input.variableDeclaration.variableDeclarator
          ) {
            variableDeclarators = [
              input.variableDeclaration.variableDeclarator,
            ];
  ...
  isForeachIteration(input) {
    if (
      input.expression &&
      input.expression.argumentList &&
      input.expression.argumentList.arguments &&
      input.expression.expression &&
      input.expression.expression.dotToken &&
      input.expression.expression.name &&
      this.extract(input.expression.expression.name) == "forEach"
    ) {
      return true;
    }
    return false;
  ...
  isReferencingStructureOrArrayKey(input) {
    if (
      input.openBracketToken &&
      input.closeBracketToken &&
  ...
  isReferencingStructureOrArrayKeyAssignment(input) {
    if (input.left && input.operatorToken && input.right) {
      if (this.extract(input.operatorToken) === "=") {
        if (
          this.isReferencingStructureOrArrayKey(input.left) &&
          !this.isReferencingStructureKeyWithString(input.left) &&
          !this.isPerformingArrayAccessWithInteger(input.left)
        ) {
          return true;
        }
      }
  ...
  isReferencingStructureKeyWithString(input) {
    if (
      input.openBracketToken &&
      input.closeBracketToken &&
      input.argumentExpression
  ...
  isPerformingArrayAccessWithInteger(input) {
    if (
      input.openBracketToken &&
      input.closeBracketToken &&
      input.argumentExpression
  ...
  isPerformingArrayAccessWithIntegerIdentifier(input) {
    if (
      input.openBracketToken &&
      input.closeBracketToken &&
      input.argumentExpression
  ...
  dumpArguments(functionArguments, updateContext) {
  ...
  typeFromValue(input) {
    ...
    if (this.isHexaInteger(value) || this.isInteger(value)) {
      ...
      if (arrayAccessPrefix != null) {
        ...
        if (
          arrayType != null &&
          arrayType.substring(0, 6) === "Array<" &&
          arrayType !== "Array<Dynamic>"
        ) {
  ...
  extract(input) {
    if (input.cachedText) {
      ...
      return this.replaceKeywords(
        this.ast.text.value.substr(input._fullStart, input._fullWidth).trim()
      );
    }
    return null;
  }

  static keywordReplacements = {
    from: "__from",
    to: "__to",
  };
  ...
  isIdentifier(str, acceptComposedIdentifier) {
  ...
  isFunctionCall(input) {
    if (
      input.argumentList ||
      (input.expression && input.expression.argumentList)
    ) {
      return true;
    }
    return false;
