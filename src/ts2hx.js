...
HXDumper.prototype.classHasStaticMethodsWithTypeParameterList = function (
  classInfo
) {
...
HXDumper.prototype.dumpClassPropertiesAssignmentsFromConstructor = function (
  element
) {
  if (
    element.callSignature &&
...
HXDumper.prototype.dumpStatements = function (
  statements,
  dontFinishWithLineBreak
) {
...
HXDumper.prototype.updateLineMappingWithInput = function (
  input,
  customFullStart
) {
  ...
  if (input.cachedText) {
    ...
    if (input.catchClause) {
      ...
      if (hasNonDynamicType) {
        ...
        this.write(
          "catch (" + this.extract(input.catchClause.identifier) + ":Dynamic) "
        );
...
HXDumper.prototype.isReferencingStructureOrArrayKeyAssignment = function (
  input
) {
...
HXDumper.prototype.isPerformingArrayAccessWithIntegerIdentifier = function (
  input
) {
  if (
    input.openBracketToken &&
